        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    produto_variation = []
                    # pegando as variaçoes do produto por cart id
                    for item in cart_item:
                        variation = item.variations.all()
                        produto_variation.append(list(variation))
                    # pegando as variações do produto no carrinho do usuário logado
...
def forgotPassword(request):
    if request.method == "POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(
                request,
                "Foi enviado uma solicitação de renovação de nova senha para seu email",
            )
            return redirect("login")
